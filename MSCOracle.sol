...
contract MSCOracle is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    modifier onlyEOA() {
    ...
    function initialize(address _msc_) public initializer {
    ...
    function genMscPrice() public virtual onlyEOA {
        if (block.timestamp - _lastUpdateTime <= MIN_UPDATE_INTERVAL) {
            return;
        }
        uint256 newPrice = mscPriceInner();
        uint256 price = newPrice;

        uint256 mscPrice15mTime = mscPriceTime(15 minutes);

        uint256 down = mscPrice15mTime * (LENGTH - DOWN_LINE) / LENGTH;

        uint256 up = mscPrice15mTime * (LENGTH + DOWN_LINE) / LENGTH;
        if(newPrice < down){
            price = down;
        }
        if(newPrice>up){
    ...
    function mscPriceInner() internal view virtual returns (uint256) {
    ...
    function mscPrice() external view virtual returns (uint256) {
        return mscPriceTime(6 hours);
    }

    function mscPriceTime(uint256 time) public view virtual returns (uint256) {
